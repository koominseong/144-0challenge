# dynasty_live_routes.py - v3.3
# 감독 개입 / 불펜 / 수비 위치 / 코치 대화
# 기존 v2 기능 유지
# =========================================
# app.py 등록:
...
def _render(save_id, live_row):
    ...
    bullpen_status = []
    if us:
        bp_map = state.get("bullpen", {}).get(us, {})
        for p in ctx[us].get("rps", []):
            e = bp_map.get(str(p["id"]), {})
            bullpen_status.append({
                "id": p["id"],
                "name": p["name"],
                "overall": p["overall"],
                "warming": bool(e.get("warming")),
                "warmup": e.get("warmup_pitches", 0),
                "required": e.get("required_pitches", state.get("bullpen_required", 15)),
    ...
    defense_view = []
    if us:
        positions = state.get("defense_positions", {}).get(us, {})
        for pos in ("P", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF"):
            pid = positions.get(pos)
            if pos == "P":
                pid = state.get("h_pitcher" if us == "home" else "a_pitcher")
            if not pid:
                continue
            p = ctx["players"].get(pid)
            if p:
                defense_view.append({"pos": pos, "id": p["id"], "name": p["name"], "overall": p["overall"]})

    coach_messages = state.get("coach_messages", [])[-8:]
    ...
    return render_template(
        ...
        pitch_control=(70 if cur_pitcher else 60),
